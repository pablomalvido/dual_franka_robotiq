...
def load_config(config_name='ses_controller_exp3.yaml'):
...
class ChainErgodicServoingProgressDriven(Node):
    ...
    def __init__(self,
                 ...
                 max_steps=1500,
                 num_modes=15,
                 config_file='ses_controller_real_exp3_high.yaml'):
        
        super().__init__("ses_controller_ring_exp3")
        ...
        self.s_feat = np.array([0]) #Monitored rod nodes indexes (From 0 to number of trackers)

        # Current value of targets, initialization
        self.r_feat = np.zeros((2, len(self.s_feat)))
        self.r_feat_all = np.zeros((2, 1)) #All the nodes
