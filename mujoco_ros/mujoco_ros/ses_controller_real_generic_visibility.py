...
class ChainErgodicServoingProgressDriven(Node):
    ...
    def __init__(self,
                 ...
                 max_steps=1500,
                 num_modes=15,
                 config_file='ses_controller_real_exp7_v1.yaml'):
        ...
        self.marker_poses_subs = self.create_subscription(Int32, "ses/aruco_count", self._marker_count_cb, 10) #Current marker poses
        ...
        # Current angle, initialization
        self.marker_count = 0.0
    ...
    def _activate_ses_cb(self, msg):
        if not self.initialized:
            ...
            self.marker_total = int(msg.ses_target)
    ...
    def _marker_count_cb(self, msg: Int32):
        self.marker_count = msg.data
        # print("Received count: " + str(self.marker_count))
    ...
    def init_conditions(self, X0, Z0):
        ...
        if self.use_prior:
            self.Phi_Apriori = self._load_prior(3) #In case we want to load the prior obtained from the previous exploration
            self.Phi_GT = self._load_prior(3) #In case we want to load the prior obtained from the previous exploration
        else:
    ...
    def _load_prior(self, index=999):
        if index == 999:
            ...
            filename = os.path.join(self.base_dir, str(index), "P_Obs_Refined.npy")
    ...
    def step(self):
        ...
        #     real_dist_sum += dist
        count_err = abs(self.marker_total - self.marker_count)
        
        lik_val = 1.0 / (1.0 + count_err / (2.0 * self.sigma_likelihood**2)) #y(x(t)) in the paper. How likely is to accomplish the task in this x
        ...
        self.metric_history.append(count_err)
        ...
        self.logs["error"].append(count_err)
        ...
        self.logs["r_feat"].append(self.marker_count)
        self.logs["r_targ"].append(self.marker_total)
        if self.iter % 30 == 0:
