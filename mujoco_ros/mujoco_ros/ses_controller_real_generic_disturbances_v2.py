"""
Progress-Variable Ergodic Control - WITH TARGET PERTURBATION

Same structure as ses_controller_rod.py (monotonic progress variable) with:
- Automatic disturbance detection via D(t) = Delta_e / (Delta_q + eps)
- On perturbation: Gaussian prior centered on current EE position,
  Bayesian accumulator reset, progress variable reset
- This gives the controller a "fresh start" in the neighborhood of
  the pre-perturbation solution for local re-exploration

Key dynamics:
1. Ergodic-settling gate: rho(E) = 1/(1 + exp(gamma(E - E_ref)))
2. Progress variable: ds/dt = rho(E)  (monotonic)
3. Schedules theta(s), T(s) parameterized by progress s
4. Disturbance detection: D(t) = (e(t)-e(t-W)) / (||q(t)-q(t-W)|| + eps)
5. Trigger: converged (sigma_D low in baseline) AND D(t) > mu_D + k*sigma_D
6. On perturbation: s->0, alpha->0, prior->Gaussian at current pos
"""

import rclpy
from rclpy.node import Node
from geometry_msgs.msg import Point, Vector3, WrenchStamped
from custom_msgs.msg import SesStarterAngle, SesStarter, MarkerPoseArray, MarkerPose
...
class ChainErgodicServoingProgressDriven(Node):
    ...
    def __init__(self,
        ...
        self.activate_ses_1D_subs = self.create_subscription(SesStarterAngle, "ses/activate_1D", self._activate_ses_1D_cb, 10) #Activate for angle targets
        #Features feedback
        ...
        self.feat_1D_subs = self.create_subscription(Float64, "ses/feat_1D", self._feat_1D_cb, 10) #Current features value for 1D case (e.g. angle or visibility)
        #Other topics
        ...
        self.r_feat = np.zeros((self.d_feat, len(self.s_feat))) # Current features values (only the tracked ones)
        self.r_feat_all = np.zeros((self.d_feat, self.n_feat)) # All the features (tracked and not tracked)
        self.r_targ = np.zeros((self.d_feat, len(self.s_feat))) # Target features values
        ...
        # ========== PERTURBATION (target redefinition + exploration reset) ==========
        self.perturbation_enabled = cfg.get('perturbation_enabled', True)

        # Gaussian prior width for post-perturbation local exploration
        self.perturbation_prior_sigma = cfg.get('perturbation_prior_sigma', 0.15)

        # ========== DISTURBANCE INJECTION (external event) ==========
        # At disturbance_time, r_targ is silently swapped to new targets.
        # This simulates an external change the controller must react to.
        total_time = self.max_steps * self.dt
        self.disturbance_step = cfg.get('disturbance_step', int(0.5 * max_steps))
        self.disturbance_time = self.disturbance_step * self.dt
        self.disturbance_injected = False

        # ========== DISTURBANCE DETECTION (automatic response) ==========
        # D(t) = (e_now - e_old) / (||q_now - q_old|| + eps)  [signed]
        # Trigger: D(t) > mu_D + k_sigma * sigma_D  (adaptive anomaly detection)
        # No fixed error or D thresholds — fully data-driven.
        self.detect_window = int(cfg.get('detect_window_sec', 1.5) / self.dt)  # samples
        self.detect_min_time = cfg.get('detect_min_time', 10.0)  # ignore early transients
        self._error_buffer = []   # sliding window for D(t) computation
        self._pos_buffer = []     # sliding window for D(t) computation

        # Adaptive baseline: rolling buffer of recent D values used to
        # estimate the "normal" level of D via mean + k*std.
        # A larger window gives more robustness; k_sigma controls sensitivity.
        self._D_baseline_window = int(cfg.get('D_baseline_window_sec', 15.0) / self.dt)
        self._D_baseline_buffer = []   # stores past D values
        self.D_k_sigma = cfg.get('D_k_sigma', 1.5)   # z-score multiplier. USED TO BE 4.0
        self.D_min_baseline = cfg.get('D_min_baseline', 20)  # min samples before triggering
        self.D_adaptive_threshold = 0.0
        self._perturbation_count_thr = cfg.get('perturbation_count_thr', 6) # consecutive pertubation events required to trigger re-exploration

        # Stability: inferred from low variability in D (quiet D -> settled system).
        # No error threshold needed — system is deemed stable when sigma_D is small
        # relative to the current mean_D and the baseline is long enough.
        self._stability_threshold = cfg.get('stability_threshold', 6.0) # stable if std(D)/mean(D) < stability_threshold
        self._confirmed_stable = False
        self.perturbation_r_targ = np.array(
            cfg.get('perturbation_r_targ', [[0]])
        )
        self.perturbation_applied = False
        self.perturbation_time = None  # set when auto-detected
        self.r_targ_original = self.r_targ.copy()
        ...
        self.logs = {
            ...
            "u_x": [], "u_z": [], "D_indicator": [], "disturbance_event": [], "reset_event": []
        }
        ...
        print("=" * 60)
        print("SES CONTROLLER (Progress-Variable) + TARGET PERTURBATION")
        print("=" * 60)
        print(f"  theta_0={self.theta_0}, lambda_theta={self.lambda_theta}")
        print(f"  T_0={self.T_0}, lambda_T={self.lambda_T}")
        print(f"  Gate: gamma={self.gate_gamma}, E_ref={self.gate_E_ref}")
        print(f"  kappa_alpha={self.kappa_alpha}")
        if self.perturbation_enabled:
            print(f"  Disturbance: t={self.disturbance_time:.1f}s (target swap)")
            print(f"  Detection: adaptive D(t) > mu_D + {self.D_k_sigma}*sigma_D")
            print(f"    D window: {self.detect_window} samples ({self.detect_window * self.dt:.1f}s)")
            print(f"    D baseline: {self._D_baseline_window} samples ({self._D_baseline_window * self.dt:.1f}s)")
            print(f"    Min trigger time: {self.detect_min_time}s")
            print(f"    New target: {self.perturbation_r_targ}")
        print("=" * 60)
    ...
    def _activate_ses_1D_cb(self, msg):
        if not self.initialized:
            #Rod target poses
            if not msg.ses_target:
                self.get_logger().warn("Received empty Target")
                return
            self.r_targ[0,0] = msg.ses_target
            #End effector initial position
            self.init_position = np.array([msg.start_position.x, msg.start_position.y, msg.start_position.z])
            self.init_conditions(msg.start_position.x, msg.start_position.z)
            self.t_init = time.time()
    ...
    def _feat_1D_cb(self, msg: Float64):
        self.r_feat[0,0] = msg.data
        self.r_feat_all[0,0] = msg.data
        # print(f"Received angle: {self.target_angle_deg:.2f} degrees")
    ...
    def compute_trajectory_coeffs(self): ##OLD
    ...
    #     """Compute time-averaged trajectory coefficients with exponential forgetting.

    #     Uses continuous exponential weights w(tau) = exp(-tau/T) instead of a hard
    #     rectangular window, eliminating the step discontinuities that arise when
    #     int(T/dt) changes by 1.
    #     """
    #     if len(self.phi_history) == 0:
    #         return np.zeros(self.num_modes_actual)
    ...
    #     # Exponential forgetting: weight each past sample by exp(-age / T)
    #     # Truncate at 4*T (weight < 2%) to keep computation bounded
    #     horizon = max(1, int(4.0 * self.T / self.dt))
    #     horizon = min(horizon, len(self.phi_history))
    #     phi = np.array(self.phi_history[-horizon:])  # oldest first

    #     # Ages: oldest sample has age (horizon-1)*dt, newest has age 0
    #     ages = np.arange(horizon - 1, -1, -1) * self.dt
    #     weights = np.exp(-ages / self.T)  # shape (horizon,)

    #     # Weighted average (no need for trapezoidal rule)
    #     w_sum = np.sum(weights)
    #     return (weights[:, None] * phi).sum(axis=0) / w_sum

    def _compute_gaussian_prior(self, center_nk, center_nl, sigma):
        """Compute a Gaussian prior centered at (center_nk, center_nl) in normalized coords."""
        Dist_Sq = (self.KN_GRID - center_nk)**2 + (self.LN_GRID - center_nl)**2
        Phi = np.exp(-Dist_Sq / (2.0 * sigma**2))
        Phi /= np.sum(Phi)
        return Phi

    def compute_disturbance_indicator(self, error, q_curr):
        """Compute disturbance indicator D(t) using adaptive anomaly detection.

        D(t) = (e_now - e_old) / (||q_now - q_old|| + eps)  [signed]

        Stability and trigger are both inferred from a rolling baseline of D:
        - mu_D, sigma_D estimated over _D_baseline_window past D values
        - _confirmed_stable: True when the baseline is full enough and
          sigma_D is small (D has been quiet -> system settled). No error
          threshold required.
        - Trigger fires when D > mu_D + k_sigma * sigma_D  (z-score anomaly)

        Returns (D, was_stable):
        - D: current signed indicator
        - was_stable: True if system is considered settled
        """
        self._error_buffer.append(error)
        self._pos_buffer.append(q_curr.copy())

        # Keep only detect_window + 1 samples
        max_len = self.detect_window + 1
        if len(self._error_buffer) > max_len:
            self._error_buffer = self._error_buffer[-max_len:]
            self._pos_buffer = self._pos_buffer[-max_len:]

        # Not enough history for D(t) yet
        if len(self._error_buffer) <= self.detect_window:
            return 0.0, False

        e_now = self._error_buffer[-1]
        e_old = self._error_buffer[0]
        q_now = self._pos_buffer[-1]
        q_old = self._pos_buffer[0]

        delta_e = e_now - e_old
        delta_q = np.linalg.norm(q_now - q_old)
        eps = 1e-6
        D = delta_e / (delta_q + eps)

        # Update rolling baseline buffer with current D
        self._D_baseline_buffer.append(D)
        if len(self._D_baseline_buffer) > self._D_baseline_window:
            self._D_baseline_buffer = self._D_baseline_buffer[-self._D_baseline_window:]

        n_base = len(self._D_baseline_buffer)
        if n_base < self.D_min_baseline:
            return D, False

        arr = np.array(self._D_baseline_buffer)
        mu_D = float(np.mean(arr))
        sigma_D = float(np.std(arr)) + eps

        # Stability: baseline is full and D variability is low
        # (sigma_D small means the signal has been quiet -> settled)
        if not self._confirmed_stable and n_base >= self._D_baseline_window:
            # Coefficient of variation < 1 with non-trivial baseline = settled
            if sigma_D / (abs(mu_D) + eps) < self._stability_threshold:
                self._confirmed_stable = True

        # Store baseline stats for use in apply_exploration_reset
        self._D_mu = mu_D
        self._D_sigma = sigma_D

        return D, self._confirmed_stable

    def inject_disturbance(self):
        """Inject disturbance: silently swap targets at scheduled time.

        This simulates an external event (e.g. target moves). The controller
        doesn't know it happened — D(t) must detect the resulting error spike.
        """
        current_time = self.t_real

        if (self.perturbation_enabled and
            not self.disturbance_injected and
            self.iter >= self.disturbance_step):

            old_targ = self.r_targ.copy()
            self.r_targ = self.perturbation_r_targ.copy()
            self.disturbance_injected = True

            print("\n" + "!" * 60)
            print(f"DISTURBANCE INJECTED at t={current_time:.1f}s")
            print(f"  Old target: {old_targ}")
            print(f"  New target: {self.r_targ}")
            print(f"  (Controller does NOT know — waiting for D(t) detection)")
            print("!" * 60 + "\n")
            return True
        return False

    def apply_exploration_reset(self, D_indicator, was_stable):
        """Reset exploration when disturbance is detected on a stable system.

        Trigger condition (threshold-free):
          was_stable AND D(t) > mu_D + k_sigma * sigma_D AND t > detect_min_time

        mu_D / sigma_D come from the rolling baseline computed inside
        compute_disturbance_indicator — no fixed D_threshold or error threshold.

        On detection:
        1. Set prior to Gaussian centered on current EE position (local exploration)
        2. Reset Bayesian accumulators (fresh evidence collection)
        3. Reset progress variable s -> 0 (theta, T recover to initial values)
        4. Reset alpha -> 0 (start from prior again)
        5. Clear trajectory history for fresh ergodic statistics
        6. Clear detection buffers (avoid re-triggering)
        """
        current_time = self.t_real

        # Adaptive trigger: D exceeds mu + k*sigma (z-score anomaly)
        mu_D = getattr(self, '_D_mu', 0.0)
        sigma_D = getattr(self, '_D_sigma', 1.0)
        self.D_adaptive_threshold = mu_D + self.D_k_sigma * sigma_D

        if (self.perturbation_enabled and
            not self.perturbation_applied and
            current_time >= self.detect_min_time and
            was_stable and
            D_indicator > self.D_adaptive_threshold):
        ...
        if self.perturbation_count>=self._perturbation_count_thr:
            # 1. Gaussian prior centered on current EE position (normalized)
            nk_curr = (self.q_curr[0] - self.K_MIN) / self.K_SPAN
            nl_curr = (self.q_curr[1] - self.L1_MIN) / self.L_SPAN
            nk_curr = np.clip(nk_curr, 0, 1)
            nl_curr = np.clip(nl_curr, 0, 1)
            self.Phi_Apriori = self._compute_gaussian_prior(
                nk_curr, nl_curr, self.perturbation_prior_sigma)

            # 2. Reset Bayesian accumulators
            self.Phi_Accum_Evidence = np.zeros((self.N_grid, self.N_grid))
            self.Phi_Accum_Visits = np.zeros((self.N_grid, self.N_grid)) + 1e-9

            # 3. Reset progress variable (restores theta, T to initial)
            self.s_progress = 0.0
            self.theta = self.theta_0
            self.T = self.T_0

            # 4. Reset mixing (start from Gaussian prior)
            self.alpha_mix = 0.0

            # 5. Clear trajectory history for fresh ergodic coefficients
            self.phi_history = []

            # 6. Clear detection buffers and unlatch stability
            self._error_buffer = []
            self._pos_buffer = []
            self._D_baseline_buffer = []
            self._confirmed_stable = False

            self.perturbation_applied = True
            self.perturbation_time = current_time  # record when detection fired

            delay = current_time - self.disturbance_time if self.disturbance_injected else float('nan')
            print("\n" + "=" * 60)
            print(f"DISTURBANCE DETECTED -> EXPLORATION RESET at t={current_time:.1f}s")
            print(f"  D(t) = {D_indicator:.2f} > {self.D_adaptive_threshold:.2f} "
                  f"(mu={mu_D:.2f} + {self.D_k_sigma}*sigma={sigma_D:.2f})")
            print(f"  Detection delay: {delay:.1f}s after disturbance")
            print(f"  Gaussian prior centered at ({nk_curr:.3f}, {nl_curr:.3f})")
            print(f"  Prior sigma: {self.perturbation_prior_sigma}")
            print(f"  Progress s -> 0, alpha -> 0, schedules restored")
            print("=" * 60 + "\n")
    ...
    def step(self):
        ...
        # Inject disturbance (silent target swap at scheduled time)
        # disturbance_event = self.inject_disturbance()

        # Compute feature error (against current r_targ, which may have just changed)
        meas_err_sq_sum = 0.0
        real_dist_sum = 0.0
        for f in range(len(self.s_feat)):
            dist = np.linalg.norm(self.r_feat[:, f] - self.r_targ[:, f])
            if self.d_feat == 1:
                meas_err_sq_sum = abs(dist)
            else:
                meas_err_sq_sum += dist**2 #abs(dist)
            real_dist_sum += dist

        # Disturbance detection (must come after error computation)
        D_indicator, was_stable = self.compute_disturbance_indicator(real_dist_sum, self.q_curr)

        # Auto-detect and reset exploration if disturbance detected
        reset_event = self.apply_exploration_reset(D_indicator, was_stable)
        ...
        u_magnitude = np.linalg.norm(u_phys)
        if u_magnitude > u_max:
            u_phys = u_phys * (u_max / u_magnitude)
        ...
        self.logs["r_feat"].append(self.r_feat_all.copy())
        self.logs["r_targ"].append(self.r_targ)
        self.logs["u_x"].append(u_phys[0])
        self.logs["u_z"].append(u_phys[1])
        self.logs["D_indicator"].append(D_indicator)
        self.logs["disturbance_event"].append(1.0 if disturbance_event else 0.0)
        self.logs["reset_event"].append(1.0 if reset_event else 0.0)
        if self.iter % 30 == 0:
        ...
        if(self.iter % 10==0):
            print(f"  Step {self.iter} | α={self.alpha_mix:.3f} | E={self.logs['E_ergodic'][-1]:.4f} | q_d=" + str(self.q_des) + f" | Error={self.logs['error'][-1]:.4f} | D={D_indicator:.1f} | D_thr={self.D_adaptive_threshold:.1f} | r_targ="+str(self.r_targ))
        ...
        if(self.iter >= self.max_steps):
            ...
            print(f"  Final T: {self.T:.4f}")
            print(f"  Final theta: {self.theta:.4f}")
            if self.disturbance_injected:
                print(f"  Disturbance injected at t={self.disturbance_time:.1f}s")
            if self.perturbation_applied:
                print(f"  Detection triggered reset at t={self.perturbation_time:.1f}s "
                      f"(delay={self.perturbation_time - self.disturbance_time:.1f}s)")
            else:
                print(f"  No disturbance detected (D never exceeded threshold)")
            print("=" * 60)
    ...
    def create_paper_figure(self, img_path):
        ...
        if self.r_targ.shape[0] == 2: #2D target
            ## Real system
        ...
        elif self.r_targ.shape[0] == 1: #Angle target
            # Convert logs to numpy arrays (safer for plotting)
            t = np.array(self.logs["t_real"])
            #angle_current = np.array(self.logs["r_feat"])
            angle_current = np.array([x[0,0] for x in self.logs["r_feat"]])
            #angle_target = np.array(self.logs["r_targ"])
            angle_target = np.array([x[0,0] for x in self.logs["r_targ"]])
            # Plot target angle (blue)
            ax_system.plot(t, angle_target, 'b-', linewidth=2,
                        label='Target Angle')
            # Plot current angle (red)
            ax_system.plot(t, angle_current, 'r-', linewidth=2,
                        label='Current Angle')
            # Styling
            ax_system.set_xlabel('Time [s]', fontsize=11)
            ax_system.set_ylabel('Angle [deg]', fontsize=11)
            ax_system.set_title('(a) Angle Tracking Over Time',
                                fontsize=12, fontweight='bold')
            ax_system.grid(True, alpha=0.3, linestyle='--')
            ax_system.legend(loc='upper right', fontsize=9, framealpha=0.9)
            # Optional: auto limits with small margin
            if len(t) > 0:
                ax_system.set_xlim([t.min(), t.max()])
                all_angles = np.concatenate((angle_current, angle_target))
                offset = 2.0  # degrees margin
                ax_system.set_ylim([all_angles.min() - offset,
                                    all_angles.max() + offset])
    ...
    def save_logs_to_json(self, filename="/ros2_ws/src/developments/dual_franka_robotiq/mujoco_ros/data/exp3_real/logged_data.json"):
        ...
        logs_serializable["_metadata"] = {
            "controller": "progress_variable_with_auto_disturbance_detection",
            "perturbation_enabled": self.perturbation_enabled,
            "disturbance_time": self.disturbance_time,
            "detection_reset_time": self.perturbation_time,
            "perturbation_type": "auto_disturbance_detection_with_gaussian_prior_reset",
            "perturbation_prior_sigma": self.perturbation_prior_sigma,
            "D_k_sigma": self.D_k_sigma,
            "D_baseline_window_samples": self._D_baseline_window,
            "detect_window_samples": self.detect_window,
            "original_targets": convert(self.r_targ_original),
            "perturbed_targets": convert(self.perturbation_r_targ),
            "theta_0": self.theta_0,
            "lambda_theta": self.lambda_theta,
            "T_0": self.T_0,
            "lambda_T": self.lambda_T,
            "gate_gamma": self.gate_gamma,
            "gate_E_ref": self.gate_E_ref
        }
...
def main():
    ...
    print("Experiment real Controller initialized. Waiting for simulation...")
    try:
