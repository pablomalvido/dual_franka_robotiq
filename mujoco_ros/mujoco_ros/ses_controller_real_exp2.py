...
def load_config(config_name='ses_controller_exp2.yaml'):
...
class ChainErgodicServoingProgressDriven(Node):
    ...
    def __init__(self,
                 ...
                 max_steps=1500,
                 num_modes=15,
                 config_file='ses_controller_real_exp2.yaml'):
        
        super().__init__("ses_controller_chain_exp2")
        ...
        self.s_feat = np.array([0,1,2]) #Monitored rod nodes indexes (From 0 to number of trackers)

        # Current value of targets, initialization
        self.r_feat = np.zeros((2, len(self.s_feat)))
        self.r_feat_all = np.zeros((2, 3)) #All the nodes
    ...
    def init_conditions(self, X0, Z0):
        ...
        if self.use_prior:
            self.Phi_Apriori = self._load_prior() #In case we want to load the prior obtained from the previous exploration
            self.Phi_GT = self._load_prior() #In case we want to load the prior obtained from the previous exploration
        else:
    ...
    def _load_prior(self, filename="/ros2_ws/src/developments/dual_franka_robotiq/mujoco_ros/data/exp2_real/P_Obs_Refined.npy"):
    ...
    def create_paper_figure(self):
        ...
        ax_system.set_xlim([-0.35, 0.15]) #We only push forward and the base is at 0.45m
        ax_system.set_ylim([-0.2, 0.35]) #The beam height is 0.55m
        ...
        plt.savefig('/ros2_ws/src/developments/dual_franka_robotiq/mujoco_ros/data/exp2_real/progress_ergodic_paper_figure.png', dpi=300, bbox_inches='tight')
        plt.close()


    def save_logs_to_json(self, filename="/ros2_ws/src/developments/dual_franka_robotiq/mujoco_ros/data/exp2_real/logged_data.json"):
        """
    ...
    def save_distribution(self, P_Obs_Refined, filename="/ros2_ws/src/developments/dual_franka_robotiq/mujoco_ros/data/exp2_real/P_Obs_Refined.npy"):
