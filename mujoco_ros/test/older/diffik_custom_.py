...
# target_beam[6] = np.array([0.54032489, 0.38736954], dtype=float) # Fake
# target_beam[7] = np.array([0.56064799, 0.43305292], dtype=float) # Fake
...
target_state_beam = fourier_estimation.get_cartesian_state(contour=target_beam, nodes=target_nodes)
...
def clockwise_directions(m):
    ...
    angles = np.linspace(np.pi/2, -np.pi/2, m, endpoint=False)
    x = np.cos(angles)
    y = np.zeros(m)
    z = np.sin(angles) 
...
def main():
    ...
    with mujoco.viewer.launch_passive(
        ...
        for i in target_nodes:
            beam_sites_id.append(model.site("site"+str(i+1)).id)

        tip_site_id = model.site("site_tip").id
        ...
        #print(vectors)
        exploration_directions = go_and_return(vectors) #size 2m
        beam_states_diff = deque(maxlen=int(m)) #m samples, 4 * fourier order + 2 
        robot_position_diff = deque(maxlen=int(m)) #m samples, 2 directions/axes
        last_beam_states = []
        last_eef_pos_R2 = []
        ...
        while viewer.is_running():
            ...
            #     data.mocap_pos[mocap_id, :3] += change_position
                ...
                change_position, beam_states_diff, robot_position_diff, last_beam_states, last_eef_pos_R2, exp_i, last_motion_time = fourier_estimation.jacobian_exploration(
                    data=data, t=time.time(), last_t = last_motion_time, dt=dt, dt_2=dt_2, 
                    R3=data.site(site_id).xpos.copy(), dR_cmd =change_position, beam_ids=beam_sites_id, 
                    dS=beam_states_diff, dR=robot_position_diff, 
                    last_S=last_beam_states, last_R=last_eef_pos_R2, 
                    m=m, d=exploration_directions, i=exp_i
                    )
                data.mocap_pos[mocap_id, :3] += change_position
                if exp_i >= (m*2):
                    explore_jac = False
            ...
            # GO TO TARGET
            elif False: #Test normal
                change_position = np.array(fourier_estimation.normal_motion_step(s=0.005, f = forces, dt=dt), dtype=float)
                #change_position = np.array(fourier_estimation.tan_motion_step(s=0.005, f = forces, dt=dt), dtype=float)
                data.mocap_pos[mocap_id, :3] += change_position
            else:
                if (time.time() - last_motion_time) >= dt_2:
                    eef_pos = data.site(site_id).xpos.copy()
                    ...
                    for site_i in range(len(beam_sites_id)):
                        ...
                        beam_nodes.append([node_pos[0], node_pos[2]])
                    #beam_states = fourier_estimation.get_fourier_states(contour=np.array(beam_nodes), order=2)
                    beam_states = fourier_estimation.get_cartesian_state(contour=np.array(beam_nodes))
                    ...
                    Q, explore_jac = fourier_estimation.estimate_deformation_matrix_regularized(beam_states_diff, robot_position_diff)
                    # print("### Q")
                    # print(Q)
                    #print('###')
                    #print(beam_states_diff)
                    ...
                    if explore_jac:
                        ...
                        delta_r = fourier_estimation.compute_velocity_command(Q, beam_states, target_state_beam, tip = data.site(tip_site_id).xpos.copy(), eef = data.site(site_id).xpos.copy(), lambda_gain=((dt/dt_2)*0.01))
                        #print(delta_r)
