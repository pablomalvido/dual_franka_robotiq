...
class RodErgodicServoingProgressDriven(Node):
    """
    ...
    def __init__(self,
        ...
        super().__init__("ses_controller")

        self.q_des_pub = self.create_publisher(Point, "ses/x_d", 10)
        self.activate_ses_subs = self.create_subscription(Bool, "ses/activate", self._activate_ses_cb, 10)
        self.node_states_subs = self.create_subscription(FloatListList, "ses/node_states", self._node_states_cb, 10)
        ...
        self.X0 = 0.4467193 #Init robot EE position after pre-tense motion
        self.Z0 = 0.488
        self.K_MIN = self.X0 - 0.001 #X
        self.K_MAX = self.X0 + 0.3
        self.L1_MIN = self.Z0 - 0.15 #Z
        self.L1_MAX = self.Z0 + 0.08
        
        self.K_SPAN = self.K_MAX - self.K_MIN
        self.L_SPAN = self.L1_MAX - self.L1_MIN
        
        # Features on End-Effector
        self.s_feat = np.array([4, 6, 8]) #Monitored rod nodes indexes (From 0 to 9)

        # Real targets
        # self.r_targ = np.array([
        #     [0.4929216, 0.53032489, 0.57120708], #X's of nodes with indexes s_feat
        #     [0.29464233, 0.38736954, 0.47863059] #Z's of nodes with indexes s_feat
        # ])

        # Imposible real targets 1
        self.r_targ = np.array([
            [0.4429216, 0.56032489, 0.6120708], #X's of nodes with indexes s_feat
            [0.29464233, 0.38736954, 0.46863059] #Z's of nodes with indexes s_feat
        ])

        # Imposible real targets 2
        # self.r_targ = np.array([
        #     [0.55, 0.6, 0.7], #X's of nodes with indexes s_feat
        #     [0.25, 0.38, 0.4] #Z's of nodes with indexes s_feat
        # ])
        ...
        self.r_feat_all = np.zeros((2, 10)) #All the nodes
        
        # Progress-Variable Dynamics
        self.gate_gamma = 5.0
        self.gate_E_ref = 0.1 #0.25
        self.s_progress = 0.0
        ...
        #self.Phi_Apriori = self._load_prior()
    ...
    def _activate_ses_cb(self, msg: Bool):
        self.ses_active = msg.data

    def _node_states_cb(self, msg: FloatListList):
        idx = 0
        f = 0
        for row in msg.rows:
            self.r_feat_all[0,idx] = row.data[0]
            self.r_feat_all[1,idx] = row.data[1]
            if idx in self.s_feat:
                self.r_feat[0,f] = row.data[0]
                self.r_feat[1,f] = row.data[1]
                f+=1
            idx +=1
    ...
    def _load_prior(self, filename="/home/rosdev/ros2_ws/src/mujoco_ros/mujoco_ros/P_Obs_Refined.npy"):
    ...
    def step(self, t=0):
        ...
        if(self.iter >= 1500):
            ...
            self.create_paper_figure()
            self.save_logs_to_json()
            self.save_distribution(P_Obs_Refined)
    ...
    def create_paper_figure(self):
        ...
        s_vec = np.linspace(0, self.L, 100)
        
        ## Real system
        # rx_real = np.array([self.kinematics_forward_real_cart(self.q_curr, s) for s in s_vec])
        
        ## Model prediction
        # rx_model = np.array([self.kinematics_forward_model_cart(self.q_curr, s) for s in s_vec])
        ...
        ax_system.plot(self.r_feat_all[0], self.r_feat_all[1], 'mo', markersize=8, markeredgewidth=2,
                        fillstyle='none', label='Real Features')
        
        # Real features
        # for f in range(len(self.s_feat)):
        #     r_f = self.kinematics_forward_real_cart(self.q_curr, self.s_feat[f])
        #     ax_system.plot(r_f[0], r_f[1], 'mo', markersize=8, markeredgewidth=2,
        #                 fillstyle='none', label='Real Features' if f == 0 else '')
        
        # # Model features
        # for f in range(len(self.s_feat)):
        #     r_f = self.kinematics_forward_model_cart(self.q_curr, self.s_feat[f])
        #     ax_system.plot(r_f[0], r_f[1], 'rs', markersize=6, alpha=0.6,
        #                 label='Model Features' if f == 0 else '')
        
        ax_system.set_xlim([0.35, 1.1]) #We only push forward and the base is at 0.45m
        ax_system.set_ylim([0.0, 0.6]) #The beam height is 0.55m
        ...
        plt.savefig('/home/rosdev/ros2_ws/src/mujoco_ros/mujoco_ros/progress_ergodic_paper_figure.png', dpi=300, bbox_inches='tight')
        plt.close()


    def save_logs_to_json(self, filename="/home/rosdev/ros2_ws/src/mujoco_ros/mujoco_ros/logged_data.json"):
        """
    ...
    def save_distribution(self, P_Obs_Refined, filename="/home/rosdev/ros2_ws/src/mujoco_ros/mujoco_ros/P_Obs_Refined.npy"):
...
def main():
    rclpy.init()
    ses_node = RodErgodicServoingProgressDriven(dt=0.05, num_modes=15)
    try:
