...
#include <custom_msgs/msg/force_controller_cmd.hpp>
 ...
 */
class CartesianForceController : public controller_interface::ControllerInterface {
 public:
 ...
 private:
  ...
  double dx_target_{0.0};
  double dy_target_{0.0};
  double dz_target_{0.0};
  std::atomic<double> force_target_{0.0};
  std::atomic<double> time_target_{0.0};

  // Current force (sensor feedback)
  double force_current_ = 0.0;

  // Maximum allowed values (limits)
  const double max_vx_ = 0.05;
  const double max_vy_ = 0.05;
  const double max_vz_ = 0.05;
  const double max_v_ = 0.4;
  const double max_force_ = 50.0;

  // Target speed and acceleration magnitude
  double speed_target_ = 0.0;
  double accel_target_ = 0.0;
  ...
  // ROS subscriber
  void targetForceCallback(const custom_msgs::msg::ForceControllerCmd::SharedPtr target);
  rclcpp::Subscription<custom_msgs::msg::ForceControllerCmd>::SharedPtr target_force_subscr_;

  void currentForceCallback(const geometry_msgs::msg::WrenchStamped::SharedPtr current);
