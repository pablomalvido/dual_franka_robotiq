...
#include <custom_franka_controllers/cartesian_acceleration_controller.hpp>
...
CartesianAccelerationController::command_interface_configuration() const {
...
/*
controller_interface::InterfaceConfiguration
CartesianAccelerationController::state_interface_configuration() const {
...
controller_interface::return_type CartesianAccelerationController::update(
  ...
  if (vx_target_test_>0.0){
    vx_target_test_ = vx_target_test_ + ax_ * dt_;
    RCLCPP_INFO(get_node()->get_logger(), "dt = %.9f", dt_);
    RCLCPP_INFO(get_node()->get_logger(), "vx_target_test = %.9f", vx_target_test_);
  }

  // Exponential smoothing: blend previous command with new target
  vx_cmd_ = (1.0 - smoothing_alpha_) * vx_cmd_ + smoothing_alpha_ * vx_target_test_; //DANGEROUS
...
CallbackReturn CartesianAccelerationController::on_init() {
  return CallbackReturn::SUCCESS;
}

CallbackReturn CartesianAccelerationController::on_configure(
  ...
  target_speed_subscr_ = get_node()->create_subscription<geometry_msgs::msg::Vector3>(
    ...
    std::bind(&CartesianAccelerationController::targetSpeedCallback, this, std::placeholders::_1));

  return CallbackReturn::SUCCESS;
}

CallbackReturn CartesianAccelerationController::on_activate(
    const rclcpp_lifecycle::State& /*previous_state*/) {
...
controller_interface::CallbackReturn CartesianAccelerationController::on_deactivate(
    const rclcpp_lifecycle::State& /*previous_state*/) {
...
void CartesianAccelerationController::targetSpeedCallback(
  const geometry_msgs::msg::Vector3::SharedPtr target)
{
  ...
  vx_target_test_ = std::clamp(static_cast<double>(target->x), -max_vx_, max_vx_);
  target_update = true;
}
...
// NOLINTNEXTLINE
PLUGINLIB_EXPORT_CLASS(custom_franka_controllers::CartesianAccelerationController,
