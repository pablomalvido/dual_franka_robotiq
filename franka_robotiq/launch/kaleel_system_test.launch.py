...
"""
Starts the robot with MoveIt (active) and a velocity controller (inactive)
Starts the force sensor node
Starts the node that coordinates the task: Change the active controller, move forward at a certain speed for a certain time or distance, stop. Write sensor data in a txt during that
"""
...
def generate_launch_description():
    ...
    #Start FT sensor publishing
    ft_sensor_node = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution([
                    FindPackageShare('nordbo_ft_sensors'), 'launch', 'run_ft_sensor.launch.py'
                ])
        )
    )
    ...
    robot_description_config = Command(
        [FindExecutable(name='xacro'), ' ', franka_xacro_file, ' hand:=false',
         ...
         ' ft_sensor:=true', ' ee_id:=""', ' hand_control:=false'])
    ...
    cartesian_acceleration_controller = ExecuteProcess(
        cmd=['ros2', 'control', 'load_controller', '--set-state', 'inactive',
                'cartesian_acceleration_controller'],
        output='screen'
    )
