...
class MultiSliderPublisher(Node):
    def __init__(self):
        ...
        # Slider A
        self.label_a = tk.Label(self.root, text="target force Z (/10)")
        self.label_a.pack()
        ...
        # Slider B
        self.label_b = tk.Label(self.root, text="Sensor force X")
        self.label_b.pack()
    ...
    def publish_target(self, value):
        ...
        self.get_logger().info(f'[A] Published to /target_wrench: {msg.wrench.force.z}')

    def publish_sensor(self, value):
        msg = WrenchStamped()
        msg.wrench.force.x = float(value)/10
        #self.publisher_b.publish(msg)
        self.get_logger().info(f'[B] Published to /tf_sensor_wrench: {msg.wrench.force.x}')
