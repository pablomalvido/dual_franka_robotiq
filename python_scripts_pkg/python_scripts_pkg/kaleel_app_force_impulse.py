...
from custom_msgs.msg import ForceControllerCmd
...
class MotionOrchestrator(Node):
    ...
    def __init__(self):
        ...
        self.cartesian_force_controller_target_pub = self.create_publisher(ForceControllerCmd, '/cartesian_force_controller/cmd', 10)
    ...
    def set_target_cartesian_speed(self, dx, dy, dz, f, t, timer=True):
        msg = ForceControllerCmd()
        msg.dx = dx
        msg.dy = dy
        msg.dz = dz
        msg.force = f
        msg.time = t
        self.cartesian_force_controller_target_pub.publish(msg)
        if timer:
            self.start_time=time.time()
            self.timer=self.create_timer(t+0.1, self.timer_callback)
    ...
    def timer_callback(self):
        ...
        #self.set_target_cartesian_speed(0.0,0.0,0.0,0.0,False) #No need to stop anything, the controller just does one push for specific time
        self.write_data = False
        self.write_txt()

        #Switch controller again
        stop_controllers = ['cartesian_force_controller']#['cartesian_velocity_example_controller']
        start_controllers = []
...
def main(args=None):
    ...
    start_controllers = ['cartesian_force_controller']#['cartesian_velocity_example_controller']
    ...
    node.set_target_cartesian_speed(1.0,0.0,0.0,5.5,1.0,True) #dx,dy,dz,force,time
